            ...
            <nav class="navbar navbar-expand-lg bg-light navbar-light shadow-lg py-3 py-lg-0 pl-3 pl-lg-5">
                ...
                <div class="collapse navbar-collapse justify-content-between px-3" id="navbarCollapse">
                    <div class="navbar-nav ml-auto py-0">
                        <a href="/homeortu" class="nav-item nav-link {{ set_active('homeortu') }}">Home</a>
                        <?php 
                            $idOrtu = Auth::user()->id;

                            $nilais = DB::table('orang_tua')
                            ->where('orang_tua.id_ortu', '=', $idOrtu)
                            ->get();
                        ?>
                        <a href="/daftarabsenortu" class="nav-item nav-link {{ set_active(['daftarabsenortu', 'absenortu']) }}">Daftar Absensi</a>
                        @foreach ($nilais as $ns)
                        <a href="{{ route('nilaianak', ['id_ortu' => $ns->id_ortu]) }}" class="nav-item nav-link {{ set_active('nilaianak') }}">Transkrip Nilai</a>
                        @endforeach
                        <a href="/profileortu" class="nav-item nav-link {{ set_active('profileortu') }}">Profile &nbsp;</a>
