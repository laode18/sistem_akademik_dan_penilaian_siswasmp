...
<body>
            ...
            <nav class="navbar navbar-expand-lg bg-light navbar-light shadow-lg py-3 py-lg-0 pl-3 pl-lg-5">
                ...
                <div class="collapse navbar-collapse justify-content-between px-3" id="navbarCollapse">
                    <div class="navbar-nav ml-auto py-0">
                        <a href="/homewalkes" class="nav-item nav-link {{ set_active('homewalkes') }}">Home</a>
                        <?php 
                            $idWalkes = Auth::user()->id;

                            $nilais = DB::table('wali_kelas')
                            ->where('wali_kelas.id_walkes', '=', $idWalkes)
                            ->get();
                        ?>
                        <a href="/absenwalkes" class="nav-item nav-link {{ set_active(['absenwalkes', 'absensiswawalkes']) }}">Absensi Siswa</a>
                        
                        @foreach ($nilais as $nt)
                            <a href="{{ route('nilaisiswawalkes', ['id_walkes' => $nt->id_walkes]) }}" class="nav-item nav-link {{ set_active('nilaisiswawalkes') }}">Nilai Siswa</a>
                        @endforeach
                        <a href="/profilewalkes" class="nav-item nav-link {{ set_active('profilewalkes') }}">Profile &nbsp;</a>
                        @foreach($nilais as $nt)
                            <img class="rounded-circle me-lg-2" class="nav-item nav-link" src="{{ url('images/'.$nt->foto) }}" alt="" style="width: 40px; height: 40px; margin-top: 1.3rem;">
                        @endforeach
    ...
    <div class="container-fluid bg-dark text-white border-top py-4 px-sm-3 px-md-5" style="border-color: rgba(256, 256, 256, .1) !important; bottom: 0; position: fixed;">
        <div class="row">
